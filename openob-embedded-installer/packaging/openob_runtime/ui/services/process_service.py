...
class OpenOBProcessManager:
    ...
    def _get_gstreamer_env(self) -> dict:
        ...
        # Add bundled gvsbuild bin (if present) so native gi/_gi dependencies can be resolved
        try:
            runtime_root = Path(self._venv_python).parent.parent
            gvs_bin = runtime_root / 'gvsbuild' / 'bin'
            if gvs_bin.exists():
                current_path = env.get('PATH', '')
                if str(gvs_bin) not in current_path:
                    env['PATH'] = str(gvs_bin) + os.pathsep + current_path
        except Exception:
            pass
